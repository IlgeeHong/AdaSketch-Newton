...
function AdaSketchNewtonGVMain(Aug)
    Verbose = Aug.verbose
    ...
    beta = Aug.beta
    nu = Aug.nu
    AdaSketchNewtonGVR = Array{AdaSketchNewtonGVResult}(undef,1)
    ...
    rep = 1
    while rep <= TotalRep
        println("AdaSketch-Newton-GV","-",rep)
        X,Lam,KKT,Alpha,grad_eval,objcon_eval,Time,IdCon,IdSing = AdaSketchNewtonGV(Max_Iter,EPS_Res,eta1,eta2,delta,beta,nu)
        if IdSing == 1
            println("Singular")
            push!(XStep[rep], X)
